...
Phase 1: Dedup (500KB raw → ~200KB indexed)
  ├── 10-11 MD files
  ├── Index (66KB)
  └── PDF extract (140KB)
  → Agent-assisted clustering
  → Build apex_popular + tag_index

Phase 2: Live Testing (IB_insync demo)
  ├── LLM runs Python examples
  ├── Validates API calls work
  └── Logs: pass/fail + execution time
  → Only working code proceeds

Add to tier metadata:
json"meta": {
  "40": "a1|100|12|base|tested:pass|exec:45ms",
  "76": "a2|87|3|var|tested:pass|exec:52ms",
  "234": "a2|87|1|var|tested:fail|error:timeout"
}



Phase 3: Confirmed DB (golden dataset)
  ├── Validated examples only
  ├── Real execution results
  └── Confidence = 1.0 (tested)
  → This becomes tier a1 (canonical)

Phase 3: Confidence Boost
json{
  "topic": "Connections",
  "tier": "a1",
  "confidence": 1.0,  // Was 0.9, now confirmed
  "tested": {
    "status": "pass",
    "runs": 15,
    "success_rate": "100%",
  ...
  "demo_account_safe": true
}


Phase 4: Real-world expansion
  ├── CLI examples
  ├── tkinter GUI examples
  └── Bug fixes from testing
  → Add to tier a2 (variations)


Phase 5: Vectorize + Train specialist
  ├── Embed confirmed dataset
  ├── Train domain specialist model
  └── Agent answers IB_insync questions
...
combined = concat([code_emb, desc_emb, tag_emb])
  ...
  project/
├── 1_raw/                   # Your current files
│   ├── index.md (66KB)
│   ├── pdk_extract.md (140KB)
│   └── guides/ (10 files)
│
├── 2_deduped/              # After agent clustering
│   ├── apex_popular.json
│   ├── tag_index.json
│   └── tier_a1_raw.json    # Unconfirmed examples
│
├── 3_testing/              # Live validation
│   ├── test_results.json   # pass/fail logs
│   ├── execution_times.json
│   └── failed_examples.json
│
├── 4_confirmed/            # Golden dataset
│   ├── apex_popular.json   # Updated with confidence
│   ├── tag_index.json
│   └── tier_a1_confirmed.json  # Only working code
│
├── 5_expanded/             # CLI + GUI examples
│   ├── tier_a2_cli.json
│   └── tier_a2_gui.json
│
└── 6_vectorized/           # Training data
    ├── embeddings.npy
    ├── metadata.json
    └── specialist_model/


Specialist agent training:
...
Start small: Dedup just connection examples first
Test early: Run 5-10 examples on demo before full batch
Track failures: Failed tests = gaps in knowledge
Version control: Keep raw + deduped + confirmed separate
